...
function OmokPlayInner({ config }: { config: OmokSetupConfig }) {
    ...
    <div className="flex flex-wrap items-center justify-between gap-2">
      <span>
        ...
        <span className="text-gray-600">
          턴 <span className="font-mono text-blue-800">{game.currentTurn}</span>
          {game.currentTurn === game.aiStone && (
            <span className="text-gray-500"> (CPU)</span>
          )}
        </span>
        <span className="mx-2 text-gray-300">·</span>
        <span className="text-gray-500">
          게임: <span className="font-mono">{game.status}</span>
        </span>
        <span className="mx-2 text-gray-300">·</span>
        <span className="text-gray-500">
          ESC:{" "}
          <span className="font-medium text-[#1967d2]">
            {workDisguiseOpen ? "게임 표시" : "업무 화면"}
          </span>
        </span>
      </span>
...
export function OmokPlayClient() {
  ...
  if (!config) {
    return (
      ...
      <SpreadsheetAppFrame
        ...
        sheetTab="오목"
        statusBar={
          <span className="text-gray-600">저장된 게임 설정이 없습니다.</span>
        }
      >
        <div
          className={`${sheetPanelInner} flex max-w-md flex-col items-center space-y-4 text-center`}
        >
          <p className="text-gray-600">세션을 불러올 수 없습니다.</p>
          <Link
            href="/games/omok/setup"
            className={`inline-flex min-w-[200px] justify-center ${sheetBtnBase}`}
          >
            setup으로 이동
          </Link>
        </div>
  ...
  return <OmokPlayInner key={JSON.stringify(config)} config={config} />;
}
